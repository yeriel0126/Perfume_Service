...
class APIClient {
    ...
    func createScentDiary(_ diary: ScentDiaryModel) async throws -> ScentDiaryModel {
        ...
        print("🔍 [시향일기 저장] 요청 시작")
        print("🔍 [시향일기 저장] URL: \(baseURL)/diaries")  // 끝에 "/" 제거
        if let bodyString = String(data: body, encoding: .utf8) {
    ...
    func unlikeDiary(diaryId: String) async throws -> UnlikeResponse {
        // DELETE /diaries/{id}/like (unlike 대신 like 엔드포인트에 DELETE 사용)
        return try await request("/diaries/\(diaryId)/like", method: "DELETE")
    }
    
    func getUserDiaries(userId: String) async throws -> [ScentDiaryModel] {
        print("🔍 [API] 사용자 일기 조회 요청: \(userId)")
        // GET /diaries?user_id={userId} 형태로 변경
        let encodedUserId = userId.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? userId
        let diaries: [ScentDiaryModel] = try await request("/diaries?user_id=\(encodedUserId)")
        print("✅ [API] 사용자 일기 조회 성공: \(diaries.count)개")
        return diaries
    }
    // 새로운 메서드 추가 - 일기 동기화용
    func syncUserDiaries(localDiaries: [ScentDiaryModel]) async throws -> [ScentDiaryModel] {
        print("🔄 [API] 일기 동기화 요청: 로컬 \(localDiaries.count)개")
        let body = try JSONEncoder().encode(["diaries": localDiaries])
        let syncedDiaries: [ScentDiaryModel] = try await request("/diaries/sync", method: "POST", body: body)
        print("✅ [API] 일기 동기화 완료: \(syncedDiaries.count)개")
    ...
    func uploadDiaryImage(diaryId: String, image: UIImage) async throws -> DiaryImageUploadResponse {
        ...
        guard httpResponse.statusCode == 200 || httpResponse.statusCode == 201 else {
            print("❌ [이미지 업로드] 실패: \(httpResponse.statusCode)")
...
struct ProfileUpdateData: Codable {
    let uid: String
    let updated_fields: [String: String]
