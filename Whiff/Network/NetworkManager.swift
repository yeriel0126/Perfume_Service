...
enum NetworkError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .invalidResponse:  // 🆕 추가된 케이스 설명
            return "잘못된 응답입니다."
        case .authenticationFailed:
            return "인증에 실패했습니다."
        case .encodingError:
...
class NetworkManager {
    ...
    func createScentDiaryWithImage(_ request: ScentDiaryRequest, image: UIImage) async throws -> ScentDiaryModel {
        ...
        // 🔐 **항상 최신 토큰으로 갱신하여 Authorization 헤더 설정**
        if let currentUser = Auth.auth().currentUser {
            print("🔍 [인증 토큰] Firebase 사용자 존재: \(currentUser.uid)")
            
            do {
                // 항상 최신 토큰 발급 (forcingRefresh: true)
                ...
                print("✅ [인증 토큰] 최신 토큰 발급 완료 (이미지+일기)")
                print("🔐 [인증 토큰] 토큰 길이: \(freshToken.count)자")
                
            } catch {
                print("❌ [인증 토큰] 최신 토큰 발급 실패: \(error)")
                throw NetworkError.authenticationFailed
            }
        } else {
            print("❌ [인증 토큰] Firebase 사용자 없음!")
            throw NetworkError.authenticationFailed
        }
    ...
    func createScentDiary(_ request: ScentDiaryRequest) async throws -> ScentDiaryModel {
        ...
        if let currentUser = Auth.auth().currentUser {
            ...
            do {
                ...
                print("✅ [인증 토큰] 최신 토큰 발급 완료")
                print("🔐 [인증 토큰] 토큰 길이: \(freshToken.count)자")
                print("🔐 [인증 토큰] Authorization 헤더 설정: Bearer \(String(freshToken.prefix(20)))...")
        ...
        do {
            ...
            // 헤더 확인 로그 추가
            if let authHeader = urlRequest.allHTTPHeaderFields?["Authorization"] {
                print("🔍 [최종 확인] Authorization 헤더: \(String(authHeader.prefix(50)))...")
            } else {
                print("❌ [최종 확인] Authorization 헤더 없음!")
            }
    ...
    func fetchScentDiaries(userId: String? = nil) async throws -> [ScentDiaryModel] {
        ...
        guard let url = urlComponents.url else {
            throw NetworkError.invalidURL  // ← APIError에서 NetworkError로 변경
        }
        ...
        if let currentUser = Auth.auth().currentUser {
            do {
                let freshToken = try await currentUser.getIDToken(forcingRefresh: false) // 목록 조회는 캐시된 토큰 사용
                urlRequest.setValue("Bearer \(freshToken)", forHTTPHeaderField: "Authorization")
                print("✅ [인증 토큰] 목록 조회용 토큰 설정 완료")
            } catch {
                print("⚠️ [인증 토큰] 목록 조회용 토큰 발급 실패: \(error)")
                // 토큰 발급 실패해도 공개 일기 조회는 계속 진행
            }
        } else {
            print("⚠️ [인증 토큰] 목록 조회 - Firebase 사용자 없음 (공개 일기만 조회)")
        }
        ...
        do {
            ...
            // 1. APIResponse 형태로 디코딩 시도
            do {
                let response = try decoder.decode(APIResponseWithPagination.self, from: data)
                print("✅ [시향 일기 목록 조회 성공] APIResponse 형태: \(response.data.count)개 일기")
                return response.data
            } catch {
                print("⚠️ [디코딩] APIResponse 실패, 직접 배열 디코딩 시도")
            }
            
            // 2. 직접 배열로 디코딩 시도
            do {
                let diaries = try decoder.decode([ScentDiaryModel].self, from: data)
                print("✅ [시향 일기 목록 조회 성공] 직접 배열: \(diaries.count)개 일기")
                return diaries
            } catch {
                print("❌ [디코딩] 배열 디코딩도 실패: \(error)")
                throw error
            }
...
// MARK: - 응답 모델 추가
struct APIResponseWithPagination: Codable {
    let message: String
    let data: [ScentDiaryModel]
