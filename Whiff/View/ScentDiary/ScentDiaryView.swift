...
struct ScentDiaryView: View {
    private let isCommentFeatureEnabled = false
        ...
        NavigationView {
            ...
            .onAppear {
                Task {
                    ...
                    // 이미지 시스템 점검 (디버깅용)
                    print("🔍 [임시 디버깅] 이미지 문제 진단 시작")
                    viewModel.checkStoredImages()
                    await viewModel.fixBrokenImageUrls()
            ...
            .onReceive(NotificationCenter.default.publisher(for: Notification.Name("RefreshDiaries"))) { _ in
                Task {
                    print("📢 [ScentDiaryView] 새로고침 알림 받음")
                ...
                Button(action: {
                    ...
                    HStack(spacing: 4) {
                        Image(systemName: viewModel.isLiked(diary.id) ? "heart.fill" : "heart")
                            ...
                            .scaleEffect(viewModel.isLiked(diary.id) ? 1.1 : 1.0)
                            .animation(.spring(response: 0.3, dampingFraction: 0.6), value: viewModel.isLiked(diary.id))
                        Text("\(diary.likes)")
                            .font(.caption)
                            .fontWeight(.medium)
                            .foregroundColor(.whiffSecondaryText2)
                            .animation(.easeInOut(duration: 0.2), value: diary.likes)
                    }
                }
                .buttonStyle(.plain)
...
// 기존 LocalImageView를 이것으로 완전히 교체
struct LocalImageView: View {
    ...
    private func loadLocalImage() {
        print("📸 [로컬 이미지 로딩] 시작: \(imageUrl)")
        
        guard let url = URL(string: imageUrl) else {
            print("❌ [로컬 이미지] 잘못된 URL: \(imageUrl)")
            hasError = true
            isLoading = false
            return
        }
        ...
        NavigationView {
            ScrollView {
                VStack(spacing: 0) {
                    ...
                    // 이미지 섹션 (있을 경우) - 기존 코드를 이것으로 완전히 교체
                    if let imageUrl = diary.imageUrl, !imageUrl.isEmpty {
                        Group {
                            if imageUrl.hasPrefix("file://") {
                                // 로컬 파일 이미지 처리
                                ...
                                // 원격 이미지 처리 (캐시 버스팅 추가)
                                let cacheBustedUrl = imageUrl.contains("?") ? "\(imageUrl)&t=\(Int(Date().timeIntervalSince1970))" : "\(imageUrl)?t=\(Int(Date().timeIntervalSince1970))"
