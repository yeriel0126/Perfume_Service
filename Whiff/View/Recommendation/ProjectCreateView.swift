...
struct ProjectCreateView: View {
    ...
    init(selectedModel: RecommendationModelType = .aiRecommendation) {  // 변경: .standard → .aiRecommendation
            ...
            VStack(spacing: 12) {
                NavigationLink(destination: PerfumePreferenceSurveyView(
                    ...
                    HStack {
                        ...
                        Text("AI 추천으로 시작하기")  // 변경: "일반 추천으로 시작하기" → "AI 추천으로 시작하기"
                            .bold()
                    }
                    .foregroundColor(.whiffWhiteText)
                    .frame(maxWidth: .infinity)
                    .padding()
                    .background(projectName.isEmpty ? Color.whiffSecondaryText2 : Color.whiffPrimary)
                    .cornerRadius(10)
                }
                .padding(.horizontal)
